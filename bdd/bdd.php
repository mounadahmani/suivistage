...
class BDD {
	...
	//cette fonction permet de se connecter au SGBD et à une base de données
		...
		$PARAM_utilisateur='rechstage'; // nom d'utilisateur pour se connecter
		$PARAM_mot_passe='rechstage'; // mot de passe de l'utilisateur pour se connecter
...
//cette fonction renvoie le resultat d'une requete sous forme d'un tableau
	...
	function prepare_select($requete,$param){
		...
		try{
			//preparation requete
			$sth=$this->connexion->prepare($requete, array(PDO::CURSOR_FWDONLY));
			$sth->execute($param);
			$tab=$sth->fetchAll();
		}
		catch(PDOException $e){
